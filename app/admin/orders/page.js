                  ...
                  <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                    ...
                    <div className="flex items-center space-x-4">
                      ...
                      {order.customerInfo?.phone && (
                        <button
                          onClick={() => {
                            const phone = order.customerInfo.phone.replace(/[^0-9]/g, '');
                            const cleanPhone = phone.startsWith('0') ? phone.slice(1) : phone;
                            const statusTexts = { pending: 'siparişiniz onay bekliyor', processing: 'siparişiniz hazırlanmaya başlanmıştır', shipped: 'siparişiniz kargoya verilmiştir', delivered: 'siparişiniz başarıyla teslim edilmiştir', cancelled: 'siparişiniz iptal edilmiştir', refunded: 'siparişiniz iade edilmiştir' };
                            const msg = `Sayın ${order.customerInfo.firstName}, siparişiniz (#${order.orderNumber}) hakkında bilgilendirme: ${statusTexts[order.orderStatus] || 'sipariş durumunuz güncellenmiştir'}. AltınÇağ Kuyumculuk`;
