    ...
    if (error.code === '42P01') {
      const { error: createErr } = await supabaseAdmin.rpc('exec_sql', {
        sql_text: `
          ...
          CREATE POLICY "Service role full access" ON rate_limits FOR ALL USING (current_setting('role') = 'service_role') WITH CHECK (current_setting('role') = 'service_role');
        `
      });
      if (!createErr) {
        _rateLimitsReady = true;
...
export async function checkAccountLockout(identifier) {
  if (!supabaseAdmin) return { locked: true, remainingSeconds: 900 };

  const tableReady = await ensureRateLimitsTable();
  if (!tableReady) return { locked: true, remainingSeconds: 900 };
...
export async function recordFailedAttempt(identifier, maxAttempts = 5, lockoutMinutes = 15) {
  if (!supabaseAdmin) return { attempts: maxAttempts, locked: true };

  const tableReady = await ensureRateLimitsTable();
  if (!tableReady) return { attempts: maxAttempts, locked: true };
