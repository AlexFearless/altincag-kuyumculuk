...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!name || !email || !password || !phone) return res.status(400).json({ error: 'Ad, e-posta, telefon ve şifre zorunludur' });
    if (typeof name !== 'string' || name.trim().length < 2 || name.length > 100) return res.status(400).json({ error: 'Geçersiz isim' });
    if (!validateEmail(String(email))) return res.status(400).json({ error: 'Geçersiz e-posta adresi' });
    if (typeof password !== 'string' || password.length < 8 || password.length > 100) return res.status(400).json({ error: 'Şifre 8-100 karakter olmalıdır' });
    ...
    if (!validatePhone(String(phone))) return res.status(400).json({ error: 'Geçersiz telefon numarası' });

    const cleanEmail = email.toLowerCase().trim();
    const { data: existingUser } = await db.from('users').select('id').eq('email', cleanEmail).single();
    if (existingUser) return res.status(409).json({ error: 'Bu e-posta adresi zaten kayıtlı' });
    ...
    const { data: user, error: insertError } = await db
      .from('users')
      .insert({
        name: sanitize(name.trim()), email: cleanEmail, password: hashedPassword,
        phone: sanitize(phone), ip_address: ip, last_login_ip: ip,
        is_active: false, email_verified: false,
        verification_code: verificationCode, verification_expires: verificationExpires,
      })
      .select('id, name, email, phone').single();

    if (insertError) return res.status(500).json({ error: 'Kayıt sırasında hata oluştu' });
    ...
    try {
      ...
      console.log('[register] Email result:', JSON.stringify(result));
      emailSent = result.success;
    } catch (e) {
      console.error('[register] Email catch error:', e?.message || e);
    }

    res.status(201).json({
      success: true, requiresVerification: true, email: cleanEmail, emailSent,
