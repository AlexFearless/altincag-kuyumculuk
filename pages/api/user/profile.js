...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!decoded) return res.status(401).json({ error: 'Geçersiz oturum' });
      ...
      const table = decoded.userType === 'admin' ? 'admins' : 'users';
      const { data: current } = await db.from(table).select('email').eq('id', decoded.id).single();
      ...
      if (email !== undefined && email !== current?.email) {
        return res.status(400).json({ error: 'E-posta değişikliği için destek talebi oluşturun' });
      }
